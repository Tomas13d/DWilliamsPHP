document.addEventListener("DOMContentLoaded", function () {
	...
	const removeFilterUrl = (key, value) => {
		...
		if (urlQuery) {
			...
			let finalURL = "";
			if (
				newURL[newURL.indexOf(key) + key.length + 1] === "&" ||
				newURL[newURL.indexOf(key) + key.length + 1] === undefined ||
				newURL[newURL.indexOf(key) + key.length + 1] === ""
			) {
				...
				if (query[0] === element.accessKey) {
					...
					values.forEach((value) => {
						if (value === element.dataset.rel) {
							...
							chipContent.appendChild(
								createChip(element.value.replaceAll("-", " "))
							);
						}
					});
				}
		...
		element.addEventListener("change", (e) => {
			...
			if (e.target.checked) {
				...
				allChips.forEach((chip) => {
					if (
						chip.firstChild &&
						chip.firstChild.innerHTML === e.target.value.replaceAll("-", " ")
					)
						chip.remove();
				});
...
/* chip */
const toggleCanvasContaiener = () => {
	let navFilter = document.querySelector("#navFilters");
	let offcanvas = document.querySelector("#offcanvasExample");
	let optionContainer = document.querySelector("#replaceble");
if (window.innerWidth < 750) {
	let parentContainer = navFilter.parentNode.parentNode;
	if(!offcanvas){
		parentContainer.removeChild(navFilter.parentNode);
		parentContainer.innerHTML = `<div class="offcanvas offcanvas-start" tabindex="-1" id="offcanvasExample" aria-labelledby="offcanvasExampleLabel">
        <div class="offcanvas-header">
            <h5 class="offcanvas-title" id="offcanvasExampleLabel">Agregar filtros</h5>
    ...
    offcanvasBody.appendChild(navFilter);
	}
} else {
	let parentContainer = navFilter.parentNode.parentNode.parentNode
	if(!optionContainer){
		parentContainer.parentNode.removeChild(navFilter.parentNode.parentNode)
		parentContainer.innerHTML = `<div class="option-filter-cont" id="replaceble"></div>`
		let replaceble = document.querySelector("#replaceble");
    	replaceble.appendChild(navFilter);
	}
}
}
...
window.onresize = toggleCanvasContaiener
